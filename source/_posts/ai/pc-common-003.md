...
## 3.3 Google 服务（In Processing）

### 3.3.1 Gmail

### 3.3.2 Google One

Google 提供的一项订阅服务，旨在整合和扩展您的 Google 账户体验，特别是提供更多的存储空间，并附带一系列额外的福利和功能。Google One 经常对美国学生提供免费的使用额度或者很大的折扣。

#### 收费方式 (Pricing)

Google One 提供多种订阅方案，通常按月或按年收费，按年订阅通常会有折扣。价格会因地区和促销活动而异，以下是常见的存储方案：

- **免费：** 15 GB (所有 Google 账户默认拥有)
    
- **基础方案：**
    - **100 GB：** 通常每月约 $1.99 或每年约 $19.99
    
- **高级方案：**
    - **2 TB：** 通常每月约 $9.99 或每年约 $99.99 (这是许多用户选择的性价比高方案)
    - **5 TB：** 通常每月约 $24.99 或每年约 $249.99
    
**支付方式：** 通常通过 Google Play 商店（Android 设备）、App Store (iOS 设备) 或直接在 Google One 网站上使用信用卡/借记卡支付。

![Google One](GoogleOne_20250528092156.png)

### 3.3.3 Gemini


### 3.3.4 Notebook LM（进行中...）

![NotebookLM](cw-image-resizer.cwg.webp)

NotebookLM 是一款人工智能研究助手，旨在帮助用户完善和整理思路。NotebookLM 能够根据用户的笔记和上传的材料整理信息并回答问题，为用户提供更高效的信息管理和知识获取体验。使用 NotebookLM 用户可以：

- 轻松上传 PDF、网站、YouTube 视频、音频文件、Google 文档或 Google 幻灯片，或发现新的资源；
- 体验 Gemini 2.0 先进的推理和交互功能，支持多种沟通方式，例如文本、图表、图像、音频以及多种语言；
- 与用户的笔记本互动，获取基于用户来源的可靠信息，并提供清晰的内联引用，确保准确性、透明度和可信度。NotebookLM 的 AI 能够自动读取并理解用户上传的文档内容，无需用户亲自阅读长篇文本。这意味着所有人可以快速获取关键信息，提高工作效率；
- 将用户的资源转换为易于理解的格式，例如学习指南、简报、音频概述、思维导图等；

#### 建立一个新的Notebook


![建立一个新的NotebookLM](NotebookLM_20250528094233.png)

增加一个来源后，NotebookLM 会自动产生摘要。

![来源摘要](NotebookLM_20250528094701.png)

整个节目分成三个部分，最左边是来源，目前 Pro 版本每个 NotebookLM 可以拥有 300 个来源。中间是一个对话窗口，你可以在这里就用户引用来源所表示的主题与 NotebookLM 进行对话和研究。右边的窗口，用户可以生成 podcast, 笔记, 学习指南等。

对于

![英文版本支持插入的功能](NotebookLM_20250528105012.png)
...
![选择待共享文件夹](gdrive_20250529112434.png)

**2. 选择知道链接的任何人**

选择知道链接的任何人，并选择**查看者**权限。这将确保所有通过该链接访问文件夹的人都只能查看文件，而不能进行编辑、评论或上传新文件。

![设置查看者权限](gdrive_20250529112657.png)

**3. 复制链接并完成**

![共享链接](gdrive_20250529113014.png)
...
GitHub 是一个面向开发者的**代码托管平台**，主要用于**版本控制**、**协作开发**和**开源项目管理**，目前该服务由 Microsoft 拥有和管理。它基于 Git 构建，Git 是一个分布式版本控制系统。GitHub 是一个提供 **Git 仓库托管服务**的平台，你可以：

- 保存和管理代码项目（Repository）；
- 与他人协作开发（Pull Request / Issues）；
- 跟踪代码变更（Version Control）；
- 部署项目（GitHub Pages, Actions）；

### 3.4.1 GitHub 的典型用途

| 用途   | 示例                                    |
| ---- | ------------------------------------- |
| 个人开发 | 管理自己的项目代码、笔记、博客等                      |
| 团队协作 | 多人开发一个软件产品，进行代码审查与合并                  |
| 开源项目 | 像 React、TensorFlow 这样的开源框架都托管在 GitHub |
| 自动部署 | 利用 GitHub Actions 自动部署网页、构建工具链        |
| 教学用途 | 教师布置编程作业，学生用 GitHub 提交作业              |

### 3.4.2 GitHub 提供的核心功能
...
- **Pull Request (PR)**：你提出对项目的修改，请求合并；
- **Issue（问题）**：提交 bug、建议或讨论事项；
- **Fork（分叉）**：复制别人项目到你账户中，自行修改开发；
